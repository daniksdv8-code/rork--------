...
export default function CardTodayScreen() {
  ...
  const cardPayments = useMemo(() => {
    return transactions
      .filter(t => isToday(t.date) && (t.type === 'payment' || t.type === 'debt_payment') && t.amount > 0 && t.method === 'card')
      .map(t => {
  ...
  const totalCard = useMemo(() => cardPayments.reduce((s, t) => s + t.amount, 0), [cardPayments]);

  const filtered = useMemo(() => {
    if (!search.trim()) return cardPayments;
    const q = search.toLowerCase();
    return cardPayments.filter(t =>
  ...
  return (
    <>
      <Stack.Screen options={{ title: 'Безнал сегодня' }} />
      <View style={styles.container}>
        <View style={styles.header}>
          <View style={styles.headerBadge}>
            <Wallet size={18} color={Colors.info} />
            <Text style={styles.headerTitle}>БЕЗНАЛИЧНЫЕ ПЛАТЕЖИ (итого: {totalCard} ₽)</Text>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  amountText: { fontSize: 15, fontWeight: '700' as const, color: Colors.info },
