...
import { Search, HandCoins, ChevronRight, Inbox } from 'lucide-react-native';
...
export default function DebtPaymentsScreen() {
  ...
  const debtPayments = useMemo(() => {
    return transactions
      .filter(t => isToday(t.date) && t.type === 'debt_payment' && t.amount > 0)
      .map(t => {
  ...
  const totalPayments = useMemo(() => debtPayments.reduce((s, t) => s + t.amount, 0), [debtPayments]);

  const filtered = useMemo(() => {
    if (!search.trim()) return debtPayments;
    const q = search.toLowerCase();
    return debtPayments.filter(t =>
  ...
  return (
    <>
      <Stack.Screen options={{ title: 'Оплаты долгов' }} />
      <View style={styles.container}>
        <View style={styles.header}>
          <View style={styles.headerBadge}>
            <HandCoins size={18} color={Colors.success} />
            <Text style={styles.headerTitle}>ОПЛАТЫ ДОЛГОВ (итого: {totalPayments} ₽)</Text>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  plateText: { fontSize: 12, color: Colors.textMuted, marginTop: 1 },
  rowBottom: { flexDirection: 'row', alignItems: 'center', marginTop: 4 },
  methodBadge: { fontSize: 11, fontWeight: '600' as const, color: Colors.textSecondary, backgroundColor: Colors.background, paddingHorizontal: 8, paddingVertical: 2, borderRadius: 6, overflow: 'hidden' },
